...
let cookies;
let domainsRecord;
let selectionWhitelist = new Set();
let persistentWhitelist;
let tab;
document.addEventListener("DOMContentLoaded", displayCookies);
const clearCookieButton = document.getElementById("Yes");
const backToBrowsingButton = document.getElementById("No");
...
whitelistButton.addEventListener("click", async () => {
    ...
    let options = { excludeDomains: [...selectionWhitelist].sort() };
    // Save storage object
...
function countKey(objArray, key, transform) {
    let counter = new Map();
...
export async function displayCookies() {
    ...
    let numberOfCookies = cookies.length;
    if (numberOfCookies) {
        ...
        for (const domain of domains) {
            ...
            listItem.addEventListener("click", () => {
                ...
                listItem.classList.toggle("checkmark");
                if (domainsRecord[domain].selected)
                    selectionWhitelist.delete(domain);
                else
                    selectionWhitelist.add(domain);
...
async function removeThisTabCookies() {
    ...
    for (cookie of cookies) {
        if (domainsRecord[cookie.domain]?.selected)
            cookiesToRemove.push(cookie);
    }
    // Remove selected cookies
    const removedCookies = await removeCookies(cookiesToRemove);
    ...
    if (removedCookies.length) {
        let counter = countKey(removedCookies, "url", key => new URL(key).hostname.replace(/^\./, ""));
...
function domainToRegExp(domain) {
